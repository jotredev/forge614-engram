import { MemoryError, type Project } from "./domain";
...
export class MemoryWorkspace {
  ...
  init(): void {
    ...
    this.config.prepare();
    const store = openWorkspaceDatabase(this.config.databasePath, true, false);
  ...
  open(readonly = false): MemoryStore {
    this.config.read();
    return openWorkspaceDatabase(this.config.databasePath, false, readonly);
  }
