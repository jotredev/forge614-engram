...
Read the existing topic with memory_search and memory_get before modifying it. Update it with its expectedVersion instead of creating a duplicate. Use one stable requestKey for each logical save and reuse it for retries; use a new requestKey for each independent observation. Saving the same information repeatedly may reinforce its search order, but does not verify that it is true.

Before ending, save a structured summary and then close the session explicitly. After compaction, recover the known sessionId; do not invent a replacement for an existing conversation or claim that the MCP transport supplies native chat IDs. If sessions are not enabled, existing memory operations remain usable. Clients decide themselves when to save; abrupt termination cannot guarantee a final save.`;
