...
export interface AssistantPaths {directory:string;config:string;hooks?:string;plugin:string;activeConfigs:string[];activePlugins:string[]}
export function resolveAssistantPaths(id:ClientId,options:AssistantOptions={}):AssistantPaths {
  ...
  const defaults:Record<ClientId,string>={
    ...
    opencode:env.OPENCODE_CONFIG_DIR||join(env.XDG_CONFIG_HOME||join(home,'.config'),'opencode'),antigravity:join(home,'.gemini','config'),
  };
  const directory=validPath(location?.configDir??defaults[id]);
  const explicit=id==='antigravity'?undefined:location?.configFile??(id==='opencode'?env.OPENCODE_CONFIG:undefined);
  let config=id==='antigravity'?join(home,'.gemini','config','mcp_config.json'):explicit??(id==='claude-code'?(location?.configDir||env.CLAUDE_CONFIG_DIR?join(directory,'.claude.json'):join(home,'.claude.json')):
    join(directory,id==='codex'?'config.toml':id==='cursor'?'mcp.json':'opencode.json'));
  ...
  return {directory,config:validPath(config),...(id==='antigravity'?{}:{hooks:join(directory,id==='claude-code'?'settings.json':'hooks.json')}),plugin,activeConfigs,activePlugins};
}
...
export function inspectAssistant(id:ClientId,options:AssistantOptions={}):AssistantDescriptor{
  ...
  const names:Record<ClientId,string[]>={'claude-code':['claude'],codex:['codex'],cursor:['cursor'],opencode:['opencode'],antigravity:platform==='win32'?['agy.exe','agy']:['agy']};
  const warnings=coverageWarnings(id,options);const evidence:string[]=[];
  const candidates=(options.path??env.PATH??'').split(platform==='win32'?';':delimiter).filter(Boolean).flatMap(dir=>names[id].map(name=>join(dir,name)));
  ...
  if(id==='antigravity'){
    ...
    }else candidates.push(join(home,'.local','bin','agy'));
  }
  ...
  try{
    ...
    configuration.paths=[paths.config,...(id==='opencode'?[paths.plugin]:paths.hooks?[paths.hooks]:[])];
  ...
  return {id,label:LABELS[id],detected:{installed:binary!==null,executable:binary,configFound,evidence},configuration,automation:{coverage:id==='antigravity'?'mcp-only':id==='cursor'?'session-only':id==='opencode'?'experimental-system-and-compaction':'session-and-prompt',warnings}};
}
