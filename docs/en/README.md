...
> **Status:** Current & Verified (545 passed, 15 skipped platform/local PG, 0 failures, 2671 assertions across 88 files on macOS ARM64 with Bun 1.3.8; native CI verification across Ubuntu, macOS, and Windows x64 Verify Run ID 35427426902 and Release Run ID 35427429725)
...
> **Central User Storage:** `~/.forge614/` (`.env` single global configuration and `engram.db` single database; automatic `0700` permission repair for existing user directories)
...
1. [**01. Installation, Setup, and Getting Started (`01-installation-and-getting-started.md`)**](01-installation-and-getting-started.md): Official installation commands (`curl`/`irm`), cryptographic verification against `SHA256SUMS`, automatic and idempotent PATH publishing (Unix and Windows), zero developer tools required for end users, interactive `setup` wizard with prior automatic `0700` permission repair and seamless assistant onboarding TUI, strict cancellation semantics, TUI Control Center (`forge614-engram tui`), 5 supported assistants, and explicit Schemas 5, 6, and 7 enablement.
...
4. [**04. TypeScript SDK Guide (`04-typescript-sdk.md`)**](04-typescript-sdk.md): TypeScript SDK guide: stable public exports from `src/index.ts`, `MemoryWorkspace`, `WorkspaceConfig` (with `repairExistingRoot()`), and `MemoryStore` facade classes in `app/`, public read-only assistant detection and path inspection API for sibling products (Atlas) with `CLIENT_IDS`, `inspectAssistant`, `resolveAssistantPaths`, and infrastructure `coverageWarnings`, and synchronous local engine architecture.
