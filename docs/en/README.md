...
4. [**04. TypeScript SDK Guide (`04-typescript-sdk.md`)**](04-typescript-sdk.md): Programmatic integration using synchronous `MemoryWorkspace` and `MemoryStore` with Schema 6 (`enableSessions`, `startSession`, `endSession`, `saveSessionSummary`, `searchPreviews`, `getVersion`, `timeline`, `context`), session types, retrieval types, and internal module boundaries.
5. [**05. Internal Architecture, MCP Protocol, and Ranking Formulas (`05-internal-architecture-and-formulas.md`)**](05-internal-architecture-and-formulas.md): SQLite Schema 6 (`sessions`, `session_entries`, `session_summaries`, `local_session_bindings`, `local_manual_sessions`), Git resolution and worktrees, MCP protocol (10 tools and Zod schemas), client adapters (`0600`/UUID backups), 3-way snapshot sync and Format 2 promotion, BM25 recency ranking formulas, and byte budget constraints.
