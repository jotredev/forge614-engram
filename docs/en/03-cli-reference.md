...
With schema 11, `session-start` adds `previous` (`{ sessionId, interruptedAt, summary }`) when the call creates the session and the project has a runtime session left open (no activity for more than 30 minutes), and `parallel` (`[{ sessionId, lastActivityAt }]`, up to 3, most recent first) when other open sessions of the project had activity in the last 30 minutes; both only when the call creates the session, and a repeated start with the same `session-id` never adds either. Since 1.7.2, when the call adds `previous`, `parallel`, or both, it also adds `sessionNotice` (text), a readable fact for the assistant derived from those two: `Session <id> was left open; its last activity was at <ISO date>; its summary is available.` (or `…; it saved no summary.`) and/or `Another session is open now: <id>.` / `Other sessions are open now: <id1>, <id2>.`; with both, `previous` comes first, joined by a space.
...
With `--format 2` the command returns, instead of that JSON, a single ready-to-inject text block for the start of an agent session, of up to 5,000 characters. Without `--format` it keeps format 1, whose output does not change; any other value answers `INVALID_INPUT` before the database is opened. The detail is in chapter [10. Startup Context for Hosts](10-startup-context.md). Available since version 1.7.0.
