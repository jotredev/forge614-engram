...
`changed` is `false` when the project was already in that group. `identityFilesUpdated` counts the `.forge614/project.json` files on this machine that were updated (the group or its name). Changing or removing the group is recorded as a local event.
