...
Con el esquema 11, `session-start` añade `previous` (`{ sessionId, interruptedAt, summary }`) cuando la llamada crea la sesión y el proyecto tiene una sesión runtime que se quedó abierta (sin actividad desde hace más de 30 minutos), y `parallel` (`[{ sessionId, lastActivityAt }]`, hasta 3, la más reciente primero) cuando otras sesiones abiertas del proyecto tuvieron actividad en los últimos 30 minutos; ambos solo cuando la llamada crea la sesión, y repetir el arranque con el mismo `session-id` nunca los añade. Desde 1.7.2, cuando la llamada agrega `previous`, `parallel` o ambos, agrega además `sessionNotice` (texto), un hecho legible para el asistente derivado de esos dos: `Session <id> was left open; its last activity was at <fecha ISO>; its summary is available.` (o `…; it saved no summary.`) y/o `Another session is open now: <id>.` / `Other sessions are open now: <id1>, <id2>.`; con ambos, `previous` va primero, unidos por un espacio.
...
Con `--format 2` el comando devuelve, en vez de ese JSON, un único bloque de texto listo para inyectar en el arranque de un agente, de hasta 5 000 caracteres. Sin `--format` sigue el formato 1, cuya salida no cambia; cualquier otro valor responde `INVALID_INPUT` antes de abrir la base. El detalle está en el capítulo [10. Contexto de Inicio para Hosts](10-contexto-de-inicio.md). Disponible desde la versión 1.7.0.
