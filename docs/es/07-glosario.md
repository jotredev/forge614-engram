...
### Flujo de Incorporación Asistida (Setup Onboarding Flow / `setup` -> `assistantTui`)
Como terminar de acondicionar una oficina e invitar inmediatamente a los colaboradores a entrar: el flujo interactivo guiado donde `forge614-engram setup` primero configura el espacio de almacenamiento SQLite y, al confirmar, transiciona de forma secuencial y limpia al menú TUI de selección de asistentes para conectar hasta 5 herramientas de IA (Claude Code, Codex, Cursor, OpenCode y Antigravity) con vista previa de rutas y confirmación explícita (cero cambios silenciosos).
