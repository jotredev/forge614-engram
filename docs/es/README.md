...
> **Estado:** Vigente y Verificado (545 superadas, 15 omitidas de plataforma/PG local, 0 fallos, 2671 aserciones en 88 archivos en macOS ARM64 con Bun 1.3.8; validación nativa en CI en Ubuntu, macOS y Windows x64 Verify Run ID 35427426902 y Release Run ID 35427429725)
...
> **Espacio central de usuario:** `~/.forge614/` (`.env` de configuración única y `engram.db` de base única; reparación automática a `0700` de carpetas existentes del usuario)
...
1. [**01. Instalación, Configuración y Primeros Pasos (`01-instalacion-y-primeros-pasos.md`)**](01-instalacion-y-primeros-pasos.md): Comandos oficiales de instalación (`curl`/`irm`), verificación contra `SHA256SUMS`, publicación automática e idempotente de PATH (Unix y Windows), cero herramientas de desarrollo para el usuario final, asistente interactivo `setup` con reparación automática previa de permisos (`0700`) y traspaso automático al selector de asistentes (onboarding), semántica de cancelación, Centro de Control TUI (`forge614-engram tui`), detección de los 5 asistentes compatibles, y habilitación de Esquemas 5, 6 y 7.
...
4. [**04. Guía del SDK de TypeScript (`04-sdk-typescript.md`)**](04-sdk-typescript.md): Guía del SDK TypeScript: exportaciones públicas estables desde `src/index.ts`, clases `MemoryWorkspace`, `WorkspaceConfig` (con `repairExistingRoot()`) y fachada `MemoryStore` en `app/`, superficie pública de detección e inspección pasiva de asistentes para productos hermanos (Atlas) con `CLIENT_IDS`, `inspectAssistant`, `resolveAssistantPaths` y `coverageWarnings` de infraestructura, y arquitectura síncrona local.
