...
20. **Validación nativa en CI e incrustación de addon de Windows completada; tareas pendientes para versión 1.0.0:**
    La validación funcional en Windows x64 (`Verify`, **Run ID `35414475529`**), así como la compilación e incrustación del módulo nativo C++ Node-API (`windows_reparse_guard.node`) dentro de los binarios autónomos de release tanto para Windows x64 como para Windows ARM64, la prueba de ejecución empaquetada fuera del repositorio (`assistant-list` bajo perfil temporal aislado sin polución de almacenamiento) y la verificación criptográfica de los 6 artefactos de release contra `SHA256SUMS` fueron ejecutadas y verificadas con éxito en GitHub Actions (**Run ID `35423226279`**, commit `7ee9de8`). No obstante, la versión v1.0.0 mantiene como pendientes de distribución: (1) validar la ejecución en una máquina Windows física limpia sin dependencias de desarrollo (sin Node, Python ni C++ instalados), (2) certificar la ausencia de dependencias externas de tiempo de ejecución C++ (*MSVC CRT*), y (3) realizar la publicación oficial mediante el envío explícito de la etiqueta de versión `v1.0.0`.
